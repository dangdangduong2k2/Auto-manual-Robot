...
static void DRBD_Demand_PID_Speed_Mode_Init(void)
{
    ...
    DRBD.Parameter.Encoder_Resolution_PPR = 1000;
...
//        Status = DRBD_Init(2, &(DRBD.Parameter));
//        Status = DRBD_Init(3, &(DRBD.Parameter));
        Status = DRBD_Demand_Enable(1);
        Status = DRBD_Demand_Enable(2);
        Status = DRBD_Demand_Enable(3);
